...
sp_diagnosis_df[sp_icd_code_column] = sp_diagnosis_df[sp_patient_id_column_in_core_table].apply(get_icd_code_from_data, args=(sp_data_df,))
...
sp_therapy_df = sp_therapy_df.drop_duplicates(subset=[npi_column_in_core_table, sp_patient_id_column_in_core_table, claims_patient_id_column_in_core_table])
...
sp_dispense_df = sp_dispense_df[[sp_patient_id_in_sp_data, prescription_written_date_sp, ship_date_sp]]
sp_dispense_df[ship_date_sp] = sp_dispense_df[ship_date_sp].apply(
    cleanse_ship_date
)

sp_dispense_df[ship_date_sp] = pd.to_datetime(sp_dispense_df[ship_date_sp], errors='coerce')
px_dispense = px_df[[claims_patient_id_in_claims, ship_date_claims, prescription_written_date_claims, 'transaction_result', 'days_supply', 'quantity']].copy()
logger.debug("Cleansing and parsing ship_date in sp_dispense")
px_dispense[ship_date_claims] = pd.to_datetime(px_dispense[ship_date_claims])
px_dispense[prescription_written_date_claims] = pd.to_datetime(px_dispense[prescription_written_date_claims])
sp_core_copy = sp_core_df.copy()
sp_core_copy = sp_core_copy.merge(sp_dispense_df, how='left', left_on=sp_patient_id_column_in_core_table, right_on='patient_id').drop(columns=['patient_id'])
sp_core_copy = sp_core_copy.merge(px_dispense, how='left', left_on=claims_patient_id_column_in_core_table, right_on='patient_id').drop(columns=['patient_id'])
sp_core_copy[sp_core_copy[ship_date_sp] == sp_core_copy[ship_date_claims]]
...
logger.info(
    f"Prescription flags | rx_flag=True: {sp_core_copy[rx_date_match_flag].sum()} "
    f"| exact_fill_flag=True: {sp_core_copy[exact_fill_date_match_flag].sum()} "
    f"| lag_fill_flag=True: {sp_core_copy[lag_fill_date_match_flag].sum()}"
)

sp_core_copy['rx_date_match_flag'] = sp_core_copy.groupby([sp_patient_id_column_in_core_table, claims_patient_id_column_in_core_table])[rx_date_match_flag].transform(lambda x: 1 if (x == 1).any() else 0)
sp_core_copy['exact_fill_date_flag'] = sp_core_copy.groupby([sp_patient_id_column_in_core_table, claims_patient_id_column_in_core_table])[exact_fill_date_match_flag].transform(lambda x: 1 if (x == 1).any() else 0)
...
sp_payor_core_df = pd.merge(sp_payor_core_df, sp_payor_df, how='left', left_on=sp_patient_id_column_in_core_table, right_on='patient_id').drop(columns=['patient_id'])
...
sp_payor_core_df = pd.merge(sp_payor_core_df, px_payor_df, how='left', left_on=claims_patient_id_column_in_core_table, right_on='patient_id').drop(columns='patient_id')
...
sp_payor_core_df = sp_payor_core_df.drop_duplicates(subset=[npi_column_in_core_table, sp_patient_id_column_in_core_table, claims_patient_id_column_in_core_table])
logger.info(
